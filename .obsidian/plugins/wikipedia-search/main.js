...
async function createNoteInFolder(app2, title, content, folderPath, overrideExisting) {
  ...
  const filePath = (0, import_obsidian5.normalizePath)(`${folderPath}/${title}.md`);
...
async function insertLink(app2, editor, settings, article, template) {
  ...
  if (template.createNote) {
    ...
    if (path == null)
      return;
    insert = `[[${path}|${settings.prioritizeArticleTitle || editor.getSelection() === "" ? article.title : editor.getSelection()}]]`;
  }
...
var FileSuggest = class extends TextInputSuggest {
  getSuggestions(inputStr) {
    ...
    abstractFiles.forEach((file) => {
      if (file instanceof import_obsidian8.TFile && file.path.toLowerCase().contains(lowerCaseInputStr)) {
        files.push(file);
      }
    });
    return files;
...
var WikipediaSearchSettingTab = class extends import_obsidian9.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian9.Setting(containerEl).setName("Search limit").setDesc("Maximum number of search results to show. (Between 1 and 500)").addText(
    ...
    new import_obsidian9.Setting(containerEl).setName("Cursor placement").setDesc("Whether or not the cursor is placed infront of the insert instead of after it.").addToggle(
    ...
    new import_obsidian9.Setting(containerEl).setName("Open article in browser").setDesc(
...
var OpenArticleModal = class extends SearchModal {
  ...
  async onChooseSuggestion(article) {
    if (
      ...
      app.plugins.enabledPlugins.has("surfing") && !this.settings.openArticlesInBrowser && import_obsidian10.Platform.isDesktopApp
    ) {
    ...
    } else if (!this.settings.showedSurfingMessage && import_obsidian10.Platform.isDesktopApp && !this.settings.openArticlesInBrowser) {
...
async function createArticleNote(app2, settings, article, template) {
  ...
  if (settings.openCreatedNotes) {
    app2.workspace.getLeaf(settings.openArticleInFullscreen ? "tab" : "split").openFile(app2.vault.getAbstractFileByPath(path));
  }
}

// src/main.ts
var WikipediaSearchPlugin = class extends import_obsidian12.Plugin {
  constructor() {
    super(...arguments);
    __publicField(this, "settings");
  }
